...
describe('CreateDirectoryRequest', () => {
  it('should define the properties: Name, ParentId, AssociationId and set them on init', () => {
    ...
    assert.strictEqual(request.ParentId, 16, 'ParentId was not set to expected value');
    assert.strictEqual(request.AssociationId, 8, 'AssociationId was not set to expected value');
  });
});
